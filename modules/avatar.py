...
elif PY_VERSION < version.parse('3.7'):
    print('Not supported Python version. At least 3.7 is required.')
    sys.exit(0)
    ...
    async def download_image(session, steamid64):
        try:
            ...
            async with session.get(image_url) as r:
                logger.info(f'Downloading {image_url} for {steamid64}...')

                with open(f'avatar/{steamid64}.jpg', 'wb') as f:
                    shutil.copyfileobj(BytesIO(await r.read()), f)
        except (aiohttp.ClientError, OSError):
            logger.info(f'Exception while downloading image for {steamid64}')
