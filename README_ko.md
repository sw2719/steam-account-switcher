...
* 중요: 처음 이 프로그램을 사용하여 로그인 하거나, 로그인 창이 표시되는 경우, 비밀번호를 입력한 후 비밀번호 기억을 꼭 체크한후 로그인 하시면 다음부터는 자동으로 로그인이 됩니다. **이 작업은 새로운 계정을 추가할 때마다 해주어야 하며, 일정 시간동안 로그인 하지 않을 경우 다시 해주어야 합니다.)**
